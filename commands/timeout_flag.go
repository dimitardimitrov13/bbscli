...
func AddBBSAndTimeoutFlags(cmd *cobra.Command) {
	AddBBSFlags(cmd)
	cmd.Flags().IntVar(&timeoutConfig.Timeout, "timeout", 0, "timeout for BBS requests in seconds [environment variable equivalent: BBS_TIMEOUT]")
...
func setTimeoutFlag(cmd *cobra.Command, args []string) error {
	if timeoutConfig.Timeout == 0 && os.Getenv("BBS_TIMEOUT") != "" {
		timeout, err := strconv.ParseInt(os.Getenv("BBS_TIMEOUT"), 10, 16)
		if err != nil {
			return err
		}
