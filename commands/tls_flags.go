...
func tlsPreHook(cmd *cobra.Command, args []string) error {
	...
	if !cmd.Flags().Lookup("skipCertVerify").Changed && os.Getenv("SKIP_CERT_VERIFY") != "" {
		...
		if err != nil {
			returnErr = NewBBSValidationError(
				cmd,
				fmt.Errorf(
	...
	if !Config.SkipCertVerify {
		if Config.CACertFile == "" {
			returnErr = NewBBSValidationError(cmd, errMissingCACertFile)
			return returnErr
		}
	...
	if (Config.KeyFile == "") || (Config.CertFile == "") {
		returnErr = NewBBSValidationError(cmd, errMissingClientCertAndKeyFiles)
		return returnErr
	}
